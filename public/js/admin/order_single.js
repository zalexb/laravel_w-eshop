...
//pagination
    function ajax_request(page) {
        ...
        if(window.location.href.split('/')[4]=='single_city'||window.location.href.split('/')[4]=='single_country'){
            var url = '/admin/order_country_city/'+id;
        }
        else
            var url = '/admin/order_single/'+id;
        ...
        $.ajax({
            ...
            data:{
              tag: window.location.href.split('/')[4]=='single_city' ? 'city' : 'country'
            },
            beforeSend:function(){

            ...
            @foreach($goods_paginated as $good)
                <div class="col-md-4 product-left p-left">
                    <div class="product-main">
                        <a href="{{route('admin_single_good',['id'=>$good->id])}}" class="mask"><img class="img-responsive zoom-img image_link" src="{{asset('images/goods/'.$good->image->link)}}" alt="{{$good->image->alt}}" /></a>
                        <div class="product-bottom">
                            <h3 class="item_name">{{$good->name}}</h3>
                            <p>Explore Now</p>
                            <h4>
                                @if(Request::is('products'))
                                    <a data-id="{{$good->id}}" class="item_add a_button{{$good->id}}" href="javascript:;">
